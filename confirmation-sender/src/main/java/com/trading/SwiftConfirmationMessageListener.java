...
public class SwiftConfirmationMessageListener {
    ...
    @JmsListener(destination = "swift.confirmation.queue", containerFactory = "jmsContainerFactory")
    ...
    private String buySellTag(AllocationReport allocationReport) {
    ...
    private Field98A tradeDate(AllocationReport allocationReport) {
    ...
    private Field90A price(AllocationReport allocationReport) {
        Field90A f90a = new Field90A();
        f90a.setPrice(allocationReport.getPrice().toString());
        return f90a;
    }

    private MT518.SequenceA sequenceA() {
